...
void CGameStateRun::OnMove()											// 移動遊戲元素
{
    ...
    for (int i = 0; i < static_cast<int>(enemy.size()); i++)
    {
        ...
        for (int j = 0; j < static_cast<int>(item.size()); j++)
            //if ((item.at(j).GetX() >= enemy.at(i).GetX() && item.at(j).GetX() <= enemy.at(i).GetX() + enemy.at(i).GetWidth()) && (item.at(j).GetY() >= enemy.at(i).GetY() && item.at(j).GetY() <= enemy.at(i).GetY() + enemy.at(i).GetHeight()) && enemy.at(i).getHasitemNum() < 2)
			if ((item.at(j).GetX() >= enemy.at(i).GetX() ))
				if(item.at(j).GetX() <= enemy.at(i).GetX() + enemy.at(i).GetWidth()) 
					if(item.at(j).GetY() >= enemy.at(i).GetY())
						if (item.at(j).GetY() <= enemy.at(i).GetY() + enemy.at(i).GetHeight()) 
							if(enemy.at(i).hasItom() < 2)
			{
